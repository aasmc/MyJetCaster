 ...
 * @param defaultOnColor The default foreground color 'on color' for [defaultColor]
...
class DominantColorState(
    ...
    private suspend fun calculateDominantColor(url: String): DominantColors? {
        ...
        // If not in cache, then calculate the swatches in the image and return the first
        // valid color
        return calculateSwatchesInImage(context, url)
            ...
            // if we found a valid swatch, wrap it in a DominantColors class
            ?.let { swatch ->
                DominantColors(
...
private suspend fun calculateSwatchesInImage(
    ...
    val r = ImageRequest.Builder(context)
        .data(imageUrl)
        // We scale the image to cover 128px x 128px (i.e. min dimension == 128 px)
